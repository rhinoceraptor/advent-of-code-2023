...
for (let i = 0; i < schematic.length; i++) {
    for (let j = 0; j < schematic.length; j++) {
        ...
        if (leftIsNum) {
            ...
            console.log(`Found ${num} left of symbol`)
            totalSum += num
        }
        if (rightIsNum) {
            let num = chaseNumberInRow(i, j + 1)
            console.log(`Found ${num} right of symbol`)
            totalSum += num
        }

        if (upLeftIsNum) {
            let num = chaseNumberInRow(i - 1, j - 1)
            console.log(`Found ${num} up left of symbol`)
            totalSum += num
        }
        if (upIsNum && !upLeftIsNum) {
            let num = chaseNumberInRow(i - 1, j)
            console.log(`Found ${num} up of symbol`)
            totalSum += num
        }
        if (upRightIsNum && !upIsNum) {
             let num = chaseNumberInRow(i - 1, j + 1)
            console.log(`Found ${num} up right of symbol`)
            totalSum += num
        }

        if (downLeftIsNum) {
            let num = chaseNumberInRow(i + 1, j - 1)
            console.log(`Found ${num} down left of symbol`)
            totalSum += num
        }
        if (downIsNum && !downLeftIsNum) {
            let num = chaseNumberInRow(i + 1, j)
            console.log(`Found ${num} down of symbol`)
            totalSum += num
        }
        if (downRightIsNum && !downIsNum) {
             let num = chaseNumberInRow(i + 1, j + 1)
            console.log(`Found ${num} down right of symbol`)
            totalSum += num
        }
    }
}
