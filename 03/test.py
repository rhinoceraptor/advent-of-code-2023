def is_symbol(char):
    """ Check if a character is a symbol (not a number or period). """
    return char not in "0123456789."

def sum_part_numbers_corrected(schematic):
    """ Corrected function to sum all part numbers in the given schematic, accounting for all edge cases. """
    sum_parts = 0
    rows = len(schematic)
        ...
        for dx in [-1, 0, 1]:
            for dy in [-1, 0, 1]:
                if dx == 0 and dy == 0:
                    continue
                nx, ny = x + dx, y + dy
                if 0 <= nx < cols and 0 <= ny < rows and is_symbol(schematic[ny][nx]):
                    return True
        return False
    ...
    for y in range(rows):
        ...
        while x < cols:
            if schematic[y][x].isdigit():
                number_start = x
                while x < cols and schematic[y][x].isdigit():
                    x += 1
                number = int(schematic[y][number_start:x])
                if any(has_adjacent_symbol(nx, y) for nx in range(number_start, x)):
                    sum_parts += number
            else:
                x += 1
...
# Replace this with your actual engine schematic
your_engine_schematic = [
...
sum_of_part_numbers = sum_part_numbers_corrected(your_engine_schematic)
